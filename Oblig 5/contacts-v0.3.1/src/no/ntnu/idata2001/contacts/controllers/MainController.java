...
public class MainController {
  ...
  private final String DATABASE_FIRST = "contacts-pu";
  private final String DATABASE_LOCAL = "contacts-pu-local-db";
  private final String DATABASE_MYSQL = "contacts-pu-mysql";
   ...
   *
   * @param addressBookDBHandler the address book to add the new contact to.
   ...
   */
  public void addContact(AddressBookDBHandler addressBookDBHandler, ContactsApp parent) {
    ...
    if (result.isPresent()) {
      ...
      addressBookDBHandler.addContact(newContactDetails);
   ...
   */
  public void editContact(ContactDetails selectedContact, ContactsApp parent, AddressBookDBHandler addressBookDBHandler) {
    if (selectedContact == null) {
      showPleaseSelectItemDialog();
    } else {
      ContactDetailsDialog contactDialog = new ContactDetailsDialog(selectedContact, true);
      Optional<ContactDetails> result = contactDialog.showAndWait();

      if (result.isPresent()) {
        ContactDetails newContactDetails = result.get();
        addressBookDBHandler.editContact(newContactDetails);
      }
      parent.updateObservableList();
    }
  }
   ...
   * @param addressBookDBHandler     the contact register to delete the selectedContact from
   * @param parent          the parent view making the call.
   */
  public void deleteContact(ContactDetails selectedContact,
                            AddressBookDBHandler addressBookDBHandler,
                            ContactsApp parent) {
    if (selectedContact == null) {
      showPleaseSelectItemDialog();
    } else {
      if (showDeleteConfirmationDialog()) {
        addressBookDBHandler.yeetusDeletus(selectedContact.getPhone());
        parent.updateObservableList();
      }
    }
  }
   ...
   *
   * @param addressBookPlain the address book to import the read contacts into
   * @param parent      the parent making the call to this method. Used for refreshing the
   *                    Observable list used by the TableView.
   */
  public void importFromCsv(AddressBookPlain addressBookPlain, ContactsApp parent) {
    ...
    if (file != null) {
      try {
        AddressBookFileHandler.importFromCsv(addressBookPlain, file);
        parent.updateObservableList();
   ...
   *
   * @param addressBookPlain the address book to export contacts from.
   */
  public void exportToCsv(AddressBookPlain addressBookPlain) {
    FileChooser fileChooser = new FileChooser();
    ...
    if (file != null) {
      try {
        AddressBookFileHandler.exportToCsv(addressBookPlain, file);
      } catch (IOException ioe) {
        Alert alert = new Alert(Alert.AlertType.ERROR);
   ...
   *
   * @param addressBookPlain the address book to save
   */
  public void saveAddressBookToFile(AddressBookPlain addressBookPlain) {
    try {
      File outFile = new File(DATA_FILE_NAME);
      AddressBookFileHandler.saveToFile(addressBookPlain, outFile);
    } catch (IOException ioe) {
      Alert alert = new Alert(Alert.AlertType.ERROR);
   ...
   */
  public AddressBookPlain loadAddressBookFromFile() {
    File inFile = new File(DATA_FILE_NAME);
    return AddressBookFileHandler.loadFromFile(inFile);
  }

  public EntityManagerFactory loadEMF()
  {
    EntityManagerFactory emf = Persistence.createEntityManagerFactory(DATABASE_FIRST);
    return emf;
  }
